# 🏖️ 30초 해변 플로깅 (beach-plogging-flash-game)

국가해안쓰레기모니터링통계(MEIS) 데이터로 만든 30초 미니게임.
쓰레기를 주우며 실제 해양 데이터를 만나고, 데이터 활성화 성과를 정량 측정한다.
...
Cloudflare 대시보드에서 이 레포를 Workers(api)와 Pages(public)에 각각 연결.
자세한 순서는 함께 제공된 배포가이드 참고.
